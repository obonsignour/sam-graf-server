...
def communitiesNames(G, community_list):
    ...
    for level in range(len(community_list)):
        ...
        # Iterate through nodes and their community IDs at the current level
        for node, community_ids in community_list[level].items():
            # Ensure community_ids is a list
            if not isinstance(community_ids, list):
                community_ids = [community_ids]
            # Iterate through the list of community IDs
            for community_id in community_ids:
                if community_id not in community_nodes_dict:
                    community_nodes_dict[community_id] = []
                community_nodes_dict[community_id].append(node)
        # Generate community names
        for community_id, community_nodes in community_nodes_dict.items():
...
def SLPA_on_one_graph(application, graph_id, graph_type, linkTypes=["all"]):
    ...
    coms = SLPA_output_format(coms.to_node_community_map())

    # Compute the communities names for each level
    communities_names = communitiesNames(G, coms)
