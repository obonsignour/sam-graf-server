...
def update_neo4j_graph(G, new_attributes_name, graph_type, model, linkTypes):
    if graph_type == "DataGraph":
        relationship_type = "IS_IN_DATAGRAPH"
    elif graph_type == "Transaction":
        relationship_type = "IS_IN_TRANSACTION"
...
def Leiden_on_one_graph(application, graph_id, graph_type, linkTypes=["all"]):
    ...
    result = community_detection_hierarchy(G.induced_subgraph([v for v in G.vs if v.index not in exclude_indices]), level=2)
    ...
    update_neo4j_graph(G, new_attributes_name, graph_type, model, linkTypes)
